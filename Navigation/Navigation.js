import React from 'react'
import { View, Text } from 'react-native'
import { NavigationContainer } from '@react-navigation/native'
import { createBottomTabNavigator } from '@react-navigation/bottom-tabs'

import TodoListsScreen from '../Screen/TodoListsScreen.js'
import TodoScreen from '../Screen/TodoScreen.js'
import HomeScreen from '../Screen/HomeScreen.js'
import SignInScreen from '../Screen/SignInScreen.js'
import SignUpScreen from '../Screen/SignUpScreen.js'
import SignOutScreen from '../Screen/SignOutScreen.js'
import { TokenContext,TodoContext, UsernameContext } from '../Context/Context'

const Tab = createBottomTabNavigator()

export default function Navigation () {//sal mais fonctionnel et je vois pas comment faire autrement je suis nul :3
    return (
      <TodoContext.Consumer>
        {([todocontext, setTodoContext]) => (
        <TokenContext.Consumer>
          {([token, setToken]) => (
            <NavigationContainer>
              {token == null ? (
                <Tab.Navigator>
                  <Tab.Screen name='SignIn' component={SignInScreen} />
                  <Tab.Screen name='SignUp' component={SignUpScreen} />
                </Tab.Navigator>
                ): (<></>)}
              {token != null && todocontext.id != 0 ? (
                <Tab.Navigator>
                  <Tab.Screen name='Home' component={HomeScreen} />
                  <Tab.Screen name='TodoLists' component={TodoListsScreen} />
                  <Tab.Screen name='Todos' component={TodoScreen} />
                  <Tab.Screen name='SignOut' component={SignOutScreen} />
                </Tab.Navigator>
                ):(<></>)}
                {token != null && todocontext.id == 0 ? (
                <Tab.Navigator>
                  <Tab.Screen name='Home' component={HomeScreen} />
                  <Tab.Screen name='TodoLists' component={TodoListsScreen} />
                  <Tab.Screen name='SignOut' component={SignOutScreen} />
                </Tab.Navigator>
                ):(<></>)}
            </NavigationContainer>
          )}
        </TokenContext.Consumer>
        )}
      </TodoContext.Consumer>
    )
  }
